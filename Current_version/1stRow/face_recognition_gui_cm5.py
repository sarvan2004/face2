#!/usr/bin/env python3
"""
Face Recognition GUI for Raspberry Pi CM5
Optimized version of the original GUI for CM5's enhanced capabilities
"""
...
class CM5FaceRecognitionGUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Face Recognition System (CM5)")
        self.root.geometry("1000x700")
        self.root.configure(bg="#f4f7fa")
        self.root.resizable(True, False)
        
        # CM5-optimized settings
    ...
    def create_widgets(self):
        ...
        logo_canvas.create_text(100, 45, text="CM5 Face Recognition", fill="#b0c4de", font=("Arial", 9, "normal"))
        ...
        subtitle = ttk.Label(header, text="Raspberry Pi CM5 Edition", style='SubHeader.TLabel')
        ...
        ToolTip(settings_btn, "View system information and API settings.")
        ...
        # Video frame panel (larger for CM5)
        self.frame_panel = tk.Label(main_frame, bg="#222", width=640, height=480)
        self.frame_panel.pack(pady=10)
        
        self.result_label = tk.Label(main_frame, text="Recognition results will appear here.", 
                                   font=("Arial", 12), bg="#f4f7fa", fg="#333")
        self.result_label.pack(pady=5)
        
        # Status bar
        status_bar = ttk.Frame(self.root, style='Card.TFrame', height=32)
        status_bar.pack(fill=tk.X, side=tk.BOTTOM)
        self.status_label = ttk.Label(status_bar, text="Ready to start face recognition", 
                                    style='Status.TLabel', anchor="w")
        self.status_label.pack(fill=tk.X, padx=10, pady=4)
        
        # Start camera and detection automatically
    ...
    def start_face_recognition(self):
        ...
        # Try different camera indices for CM5
    ...
    def recognition_loop(self):
        """Main recognition loop optimized for CM5"""
        while not self.stop_event.is_set():
            ...
            time.sleep(0.3)  # Faster loop for CM5
    ...
    def show_attendance_popup(self, frame, name, attendance):
        ...
        cv2.rectangle(display_img, (0, 0), (640, 40), color, -1)
    ...
    def capture_face_with_detection(self, person_name):
        ...
        def update_frame():
            ...
            temp_path = "/tmp/capture_frame.jpg"
            cv2.imwrite(temp_path, frame)
    ...
    def save_captured_face(self, face_image, person_name):
        """Save the captured face to known_faces directory"""
        try:
    ...
    def upload_face_to_cloud(self, filepath, filename):
        """Upload the captured face to the cloud server"""
        try:
            # For now, just show a message about manual upload
            messagebox.showinfo("Upload", 
                              f"Face saved locally as {filename}\n\n"
                              f"To upload to cloud server, use:\n"
                              f"scp {filepath} {CLOUD_USER}@{CLOUD_IP}:{CLOUD_KNOWN_FACES_PATH}")
        except Exception as e:
    ...
    def view_attendance(self):
        ...
        try:
            ...
            if response.status_code == 200:
                with open("/tmp/attendance_downloaded.csv", "wb") as f:
                    f.write(response.content)
                df = pd.read_csv("/tmp/attendance_downloaded.csv")
